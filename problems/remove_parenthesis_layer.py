...
# and a close parenthesis will never start first), remove the
# outermost layers of the parenthesis string and return the new parenthesis string.
#
...
def remove_outermost_parenthesis(s):
    ...
    ret = ''
    for i, c in enumerate(s):
        if i == openings[index] and index < len(openings):
            index += 1
            continue
        else:
            ret += c
